...
def get_bootstrap(request):
    return render(request, 'exemplo/16_forms_parte_i.html')
...
def validou_senha(senha):
    ...
    # tem que ter letras e numeros e caracteres especias
    # Minimo 6 e maximo 15 caracteres
...
def processa_formulario_v1(request):
    if not request.POST.get("email") and  request.POST.get("senha") :
        return render(request, 'exemplo/16_forms_parte_i.html')
    ...
    if validou_form(email, senha):
        ...
        return render(request, 'exemplo/16_forms_parte_i.html', context)
...
def processa_formulario_v2(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("Deu ruim!!!!")
    return render(request, 'exemplo/17_forms_parte_ii.html', {'form': form})
